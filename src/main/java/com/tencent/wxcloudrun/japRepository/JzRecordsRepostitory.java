...
public interface JzRecordsRepostitory extends CrudRepository<JzRecords, Long> {
    ...
    @Query(value = "select jr.rec_type,cast(sum(jr.rec_money) as char ) total from jz_records jr " +
            " where jr.rec_ownerid =?1 group by jr.rec_type" ,nativeQuery = true)
    List<Map<String, Object>> queryRecordsGroupWithOpenid(String openid);
    ...
    @Query(value = "select jr.rec_date, " +
            ...
            " from jz_records jr group by jr.rec_date order by jr.rec_date DESC " ,nativeQuery = true)
    List<Map<String,Object>> queryRecordsGroupByDay();
    ...
    @Query(value = "select * from jz_records jr left join \n" +
            ...
            " where rec_date =?1 order by jr.rec_date desc,jr.rec_born_time desc",nativeQuery = true)
    List<Map<String, Object>> queryDayRecordsByDay(String day);
}
